$(document).ready(function() 
{
	...
	/* View screen starts */
		...
		$('#viewEdit').on('pageinit', function() 
		{
			...
			{			
					...
					$(document).delegate("#removeUserYes",'click',function()
					{	
					...
					$(document).delegate("#removeUserNo",'click',function()
					{
						   $("#removeUserDialog").dialog("close");
					});
	...
	/* Search screen Starts */
		...
		$('#searchMatrimonial').on('pageinit', function()
		{
			...
			$(document).delegate('#searchButton', 'click', function()
			{
			   ...
			   validate = true;
						...
						else 
						{
							...
							else
							{
								...
								for ( counter= 0; counter < searchMap.length; counter++) 
								{
									...
									newRow = '<div class="col-sm-6"><div class="user-detail"><div class="user-pic"><img src="/uravupaalamsource/matrimony/getImage/'+ProfilePicture.profilePictureFile+'" width="130" height="130"><a class="liked likeProfile" id="like'+ProfilePicture.matrimonial_id+'"></a></div><div class="usr-info"><h3><a class="viewProfile" id="'+ProfilePicture.matrimonial_id+'">'+ProfilePicture.name+'</a></h3><ul><li><div class="info-hed">Occupation</div><div class="info-value">'+ProfilePicture.occupation+'</div></li><li><div class="info-hed">Date Of Birth </div><div class="info-value">'+ProfilePicture.dob+'</div></li><li><div class="info-hed">City</div><div class="info-value">'+ProfilePicture.city+'</div></li><li><div class="info-hed">Country</div><div class="info-value">'+ProfilePicture.country+'</div></li></ul></div></div></div>';
				...
				function getPageNumber(page)
				{
					$.ajax({
					 type: 'GET',
					 url: baseUrl+'/uravupaalamsource/matrimony/matrimonialDetails?minAge='+smiage+'&maxAge='+smaage+'&gender='+sgender+'&pageNo='+page,
					 dataType: 'json',
					 encode: true,
					 success: function(response) 
					 {
				...
				$(document).delegate('.likeProfile','click',function(evt)
				{
					...
					for(counter = 0; counter < likeProfileDetails.length; counter++)
					{
						...
						{
							alertMessage("This profile is already liked ");
			...
			$(document).delegate('#searchMatrimonial :input', 'click', function()
			{
				$(this).css('border', ' 1px solid #d8e1b6');
				$("#fillMessage3").hide();
			});
	...
	/* Favourate Profile Details screen Starts */
		...
		function likedProfiles(censusPersonId)
		{
			...
			$.ajax({
				 ...
				 success: function(response) 
				 {
					...
					else 
					{
						...
						{
							likedProfileDetails = likedMap[counter];
							
							newRow = '<div class="col-sm-6"><div class="user-detail"><div class="user-pic"><img src="/uravupaalamsource/matrimony/getImage/'+likedProfileDetails.profilePictureFile+'" width="130" height="130"><a class="unLiked liked active " id="like'+likedProfileDetails.matrimonial_id+'"></a></div><div class="usr-info"><h3><a class="viewLikedProfiles" id="'+likedProfileDetails.matrimonial_id+'">'+likedProfileDetails.name+'</a></h3><ul><li><div class="info-hed">Occupation</div><div class="info-value">'+likedProfileDetails.occupation+'</div></li><li><div class="info-hed">Date Of Birth </div><div class="info-value">'+likedProfileDetails.dob+'</div></li><li><div class="info-hed">City</div><div class="info-value">'+likedProfileDetails.city+'</div></li><li><div class="info-hed">Country</div><div class="info-value">'+likedProfileDetails.country+'</div></li></ul></div></div></div>';
		...
		$('#likeProfiles').on('pageinit', function()
		{
			...
			$(document).delegate('.unLiked', 'click', function(evt) 
			{			
					
					var clicked = evt.target.id;
					
					var likedMatrimonialId = clicked.replace("like", "");
					...
					$(document).delegate("#unlikeUserYes",'click',function()
					{	
							$("#like"+likedMatrimonialId).removeClass('active');
							
							$("#likeDialog").dialog("close");
							
							for(counter = 0;counter < likedMap.length; counter++)
							{
								matrimony = likedMap[counter].matrimonial_id;
								
								if(likedMatrimonialId == matrimony)
								{
									matrimonyId = matrimony;
									break;
								}
							}
							...
							$.ajax({
							type: 'POST',
							url: baseUrl+'/uravupaalamsource/matrimony/profileStatus',
							dataType: 'json',
							data:formData9,
							encode: true,
							success: function(response) 
							{
								...
								{
									profileChangeStatusMap = response;
					...
					$(document).delegate("#unlikeUserNo",'click',function()
					{
						   $("#likeDialog").dialog("close");
					});
